export default {
    ...
    riskControl: {
      ...
      payloadAdvanced: "Advanced: custom request (optional)",
      payloadOptionalHint: "JavaScript builds the request format; it is not a moderation model or an extra policy. Use it only for providers requiring a special request format. Keep audit rules and output instructions in the system prompt.",
      defaultPayloadActive: "Using the default request. No JavaScript is needed.",
      customPayloadActive: "Custom request code is configured and will build audit requests. Expand to restore the default request.",
      resetPayload: "Restore default request",
      auditConfidenceHit: "The model score {score} reached the threshold {threshold} used in this test, so the result is flagged.",
      auditConfidenceMiss: "The model score {score} is below the threshold {threshold} used in this test, so the result is not flagged.",
      auditBooleanDecision: "The model returned flagged={value} without a confidence score. This boolean determines the result; no threshold comparison applies.",
      auditTestDraftHint: "This is a snapshot of the test decision. Live blocking also depends on saved switches, mode, scope and sample rate.",
      chatTimeoutHint: "Timeouts fail open under the current policy. Allow headroom based on test latency; chat and dedicated moderation models may have different response times.",
      customCode: "Custom moderation",
      customCodeTypeSafe: "These editors apply to OpenAI-compatible APIs. Select OpenAI Compatible in Basic settings. TypeSafe AI keeps its existing rules.",
      apiFormat: "Moderation API format",
      apiFormatHint: "Choose Chat Completions for general chat models, or Moderations for dedicated moderation models such as omni.",
      confidenceThreshold: "Confidence threshold",
      confidenceThresholdHint: "Enter 0–1, e.g. 0.85 for 85%. A returned confidence at or above this value is flagged; this setting does not change the model score. A standalone flagged boolean is used directly. When both are present, confidence takes precedence.",
      auditPrompt: "System prompt",
      auditPromptPlaceholder: "Paste your moderation system prompt here.",
      auditPromptHint: "The default request sends this as the system prompt. Use one consistent JSON output format with confidence (0–1) and reason. Required for Chat Completions; native Moderations does not use system prompts.",
      payloadScript: "Request payload code (JavaScript, optional)",
      payloadScriptPlaceholder: "Usually leave this empty. Define const requestBody = ... only when a custom request format is required.",
      payloadScriptHint: "Variables: text (audited text), input (original moderation input including images), config.model, config.auditPrompt, isModerationEndpoint. Define requestBody as a JSON string or object. The server sends it; no fetch is needed. No network or file APIs; execution limit: 100 ms. Scripts using only text omit images.",
      insertPayloadExample: "Insert default request example",
      customCodeTestHint: "Set the provider URL, model, Key and system prompt, then run an audit test. Tests use unsaved settings; the live gateway uses saved settings. API errors and timeouts retain the existing fail-open behavior.",
      goToAuditTest: "Go to Basic settings and audit test",
      chatThresholdsHint: "Chat moderation uses the confidence threshold in Custom moderation. Existing category thresholds apply only to Moderations and TypeSafe AI.",
      engine: 'Audit engine',
      ...
      typeSafeNotice: "TypeSafe AI uses its own API and built-in rules, with jev-latest as its preset model. It does not use the Custom moderation system prompt. It audits text only and does not send images. Switching engines keeps shared blocking, notification and ban settings.",
