export default {
    ...
    riskControl: {
      ...
      confidenceThresholdHint: "返回 confidence 时，达到此值即命中；仅返回 flagged 时直接使用其布尔值。两者都有时以 confidence 为准。",
      auditPrompt: "系统提示词",
      auditPromptPlaceholder: "在这里粘贴你的审核系统提示词。",
      auditPromptHint: "通过 config.auditPrompt 提供给 payload。Chat Completions 模式必填；Moderations 原生接口不接受系统提示词。",
      payloadScript: "请求 payload 代码（JavaScript）",
      payloadScriptPlaceholder: "在这里粘贴 const wrappedUserContent = ... 和 const requestBody = ... 代码。留空使用默认请求。",
      ...
      insertPayloadExample: "填入 payload 示例",
      customCodeTestHint: "模型需输出含 confidence（0–1）或 flagged（布尔值）的 JSON，可附 reason。请在「基础设置」填写服务商地址、模型和 Key，并用「审核测试」验证；测试会使用当前未保存的代码。审核接口失败仍按原有策略放行。",
