export default {
    ...
    riskControl: {
      ...
      payloadAdvanced: "高级：自定义请求（可选）",
      payloadOptionalHint: "JavaScript 是组装请求格式的代码，不是审核模型或额外的审核规则。仅在服务商需要特殊请求格式时使用；审核规则和输出格式请统一写在系统提示词中。",
      defaultPayloadActive: "当前使用默认请求，无需编写 JavaScript。",
      customPayloadActive: "当前已填写自定义请求代码，将按该代码发送审核请求。可展开后恢复默认请求。",
      resetPayload: "恢复默认请求",
      auditConfidenceHit: "本次模型分数 {score}，达到本次试跑阈值 {threshold}，判定命中。",
      auditConfidenceMiss: "本次模型分数 {score}，低于本次试跑阈值 {threshold}，判定未命中。",
      auditBooleanDecision: "模型直接返回 flagged={value}，未提供置信度；本次按布尔结果判定，不比较阈值。",
      auditTestDraftHint: "这是本次试跑的判定快照。实际拦截还取决于已保存的开关、模式、审核范围和采样率。",
      chatTimeoutHint: "超时会按现有策略放行。请根据试跑延迟预留时间；聊天模型与专用审核模型的耗时可能不同。",
      customCode: "自定义审核",
      customCodeTypeSafe: "这两个编辑框用于 OpenAI 兼容接口。请在「基础设置」中选择 OpenAI Compatible 引擎；TypeSafe AI 沿用其原有审核规则。",
      apiFormat: "审核接口类型",
      apiFormatHint: "通用聊天模型选择 Chat Completions；omni 等专用审核模型选择 Moderations。",
      confidenceThreshold: "拦截置信度阈值",
      confidenceThresholdHint: "填写 0–1，例如 0.85 表示 85%。模型返回的 confidence 达到此值即命中；这个阈值不会改变模型评分。仅返回 flagged 时直接使用其布尔值；两者都有时以 confidence 为准。",
      auditPrompt: "系统提示词",
      auditPromptPlaceholder: "在这里粘贴你的审核系统提示词。",
      auditPromptHint: "默认请求会将这里的内容作为系统提示词发送。建议统一要求只输出 confidence（0–1）和 reason 的 JSON。Chat Completions 必填；Moderations 原生接口不使用系统提示词。",
      payloadScript: "请求 payload 代码（JavaScript，可选）",
      payloadScriptPlaceholder: "通常留空即可。只有需要自定义请求格式时，才填写 const requestBody = ... 代码。",
      payloadScriptHint: "可用变量：text（待审文字）、input（含图片的原始审核输入）、config.model、config.auditPrompt、isModerationEndpoint。定义 requestBody 为 JSON 字符串或对象；服务端负责发送，无需 fetch。脚本无网络和文件 API，执行限时 100 ms。仅使用 text 的脚本不会包含图片。",
      insertPayloadExample: "填入默认请求示例",
      customCodeTestHint: "先填写服务商地址、模型、Key 和系统提示词，再用审核测试验证。测试使用当前未保存的设置；实际网关使用已保存的设置。审核接口报错或超时仍按原有策略放行。",
      goToAuditTest: "前往基础设置与审核测试",
      chatThresholdsHint: "当前为聊天模型审核，使用「自定义审核」中的置信度阈值；下方原有的分类阈值仅适用于 Moderations 和 TypeSafe AI。",
      engine: '审计引擎',
      activeEngine: '当前生效引擎：{engine}',
      engineUnavailable: '当前引擎没有可用 Key，API 审计不可用；失败请求按现有策略放行。',
      typeSafeNotice: "TypeSafe AI 使用专用接口和内置审核规则，预设模型为 jev-latest，不使用「自定义审核」中的系统提示词。仅审核文字，图片不外发；切换引擎不会关闭公共拦截、通知或封禁设置。",
