  ...
  <article v-for="p in config.providers" :key="p.id" class="space-y-3 rounded-xl border border-gray-200 p-4 dark:border-dark-700">
    <h3 class="font-medium">{{ p.name || p.id }} · {{ p.model }}</h3>
          ...
          <details v-for="(p, index) in config.providers" :key="p.id" class="rounded-xl border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-800" :open="expanded === p.id">
            ...
            <div class="space-y-4 border-t border-gray-100 p-4 dark:border-dark-700">
              ...
              <details class="rounded-lg bg-gray-50 p-3 dark:bg-dark-900/50">
                <summary class="cursor-pointer text-sm">{{ t('moderationV2.generationSettings') }}</summary>
        ...
        <div v-if="section === 'policy'" class="space-y-5">
          <label class="field">{{ t('moderationV2.auditMode') }}<select v-model="policy.mode" class="input" data-test="audit-mode"><option value="legacy">{{ t('moderationV2.legacyMode') }}</option><option value="balanced">{{ t('moderationV2.balancedMode') }}</option><option value="quality">{{ t('moderationV2.qualityMode') }}</option></select></label>
          <p class="hint">{{ t('moderationV2.modeHint') }}</p>
          <div v-if="policy.mode !== 'legacy'" class="space-y-4 rounded-xl border border-primary-200 p-4 dark:border-primary-900">
            <label class="field">{{ t('moderationV2.contextMode') }}<select v-model="policy.context_mode" class="input" data-test="context-mode"><option value="bounded">{{ t('moderationV2.boundedContext') }}</option><option value="full">{{ t('moderationV2.fullContext') }}</option><option value="current">{{ t('moderationV2.currentContext') }}</option></select></label>
            ...
            <details><summary class="cursor-pointer text-sm">{{ t('moderationV2.contextLimits') }}</summary><div class="mt-4 grid gap-4 md:grid-cols-2">
