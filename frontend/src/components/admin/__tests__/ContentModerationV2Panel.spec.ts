...
describe('channels inside risk-control settings', () => {
  ...
  it('exposes context and shared budgets only after opting into enhanced review', async () => {
    ...
    expect(w.props('modelValue').unresolved_policy).toBe('reject_temporary')
  ...
  it('keeps advanced channel settings and credentials when editing inline rules', async () => {
    const model = fixture()
    Object.assign(model.providers[0]!, { api_format: 'responses', timeout_ms: 75000, max_input_tokens: 16000, payload_script: 'const requestBody = {};', audit_validated: false })
    const before = structuredClone(model.providers[0]!)
    const w = await render('services', model)
    expect(w.get('[data-test="provider-advanced"]').attributes('open')).toBeUndefined()
    await w.get('[data-test="channel-prompt"]').setValue('Updated rules')
    await w.setProps({ section: 'policy' })
    expect(model.policy!.mode).toBe('legacy')
    await w.setProps({ section: 'services' })
    expect(model.providers[0]).toEqual(expect.objectContaining({ ...before, audit_prompt: 'Updated rules' }))
    expect(moderationV2API.save).not.toHaveBeenCalled()
  })
  it('requires saving changed settings before estimating or paying for a test', async () => {
    const w = await render('trial')
    await w.setProps({ configSaved: false })
    await w.get('[data-test="v2-test-text"]').setValue('Keep this sample')
    expect(w.get('[data-test="preview-v2"]').attributes('disabled')).toBeDefined()
    expect(w.get('[data-test="test-v2"]').attributes('disabled')).toBeDefined()
    await w.get('[data-test="save-before-trial"]').trigger('click')
    expect(w.emitted('save')).toHaveLength(1)
    expect(moderationV2API.test).not.toHaveBeenCalled()
    expect(moderationV2API.preview).not.toHaveBeenCalled()
    await w.setProps({ configSaved: true, saving: false })
    expect(w.find('[data-test="trial-needs-save"]').exists()).toBe(false)
    expect(w.get('[data-test="test-v2"]').attributes('disabled')).toBeUndefined()
    expect((w.get('[data-test="v2-test-text"]').element as HTMLTextAreaElement).value).toBe('Keep this sample')
    expect(moderationV2API.test).not.toHaveBeenCalled()
  })
  it('does not display an old verdict when configuration changes during a test', async () => {
    let finish!: (result: Awaited<ReturnType<typeof moderationV2API.test>>) => void
    vi.mocked(moderationV2API.test).mockReturnValueOnce(new Promise(resolve => { finish = resolve }))
    const w = await render('trial')
    await w.get('[data-test="v2-test-text"]').setValue('Sample')
    await w.get('[data-test="test-v2"]').trigger('click')
    await w.setProps({ configSaved: false })
    finish({ status: 'reviewed', reason: '', cache_hit: false, attempts: 1, estimated_input: 10 })
    await flushPromises()
