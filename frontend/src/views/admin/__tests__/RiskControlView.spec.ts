...
describe('admin RiskControlView', () => {
  ...
  it('edits channels inside the original dialog and saves through the original endpoint', async () => {
    ...
    expect(wrapper.get('[data-test="audit-path-hint"]').text()).toBe('moderationV2.newPathHint')
    expect(wrapper.findAll('button').some(button => button.text().includes('admin.riskControl.tabs.runtime'))).toBe(false)
    await wrapper.get('[data-test="advanced-settings"]').trigger('click')
  ...
  it('preserves both engine drafts and saves thresholds independently', async () => {
    ...
    if (wrapper.get('[data-test="advanced-settings"]').attributes('aria-expanded') === 'false') await wrapper.get('[data-test="advanced-settings"]').trigger('click')
